...
template <std::size_t p=1,bool rnd=true,bool mf=false,bool prt=true,
	  bool rndtest=false,
  ...
  while(true)
    {
      ...
      const number rlx = 8./9. ;
      const auto t = 0u ;
      const auto M = make_multigrid<std::size(res),p,t,mf,gal,prt>(A);
      while (true)
      	{
      	  res = rhs - A*x;                       //                f - A x
	  ...
	  if constexpr (prt) std::cout << "rlx:" << std::left << std::setw(10)
      	  ...
      	  ++it ;
	  x += M.solve(res,rlx);                // x_1 = x_0 + B (f - A x_0)
	  if (it == 101) break ;
      	}
...
constexpr auto gmres(const auto & AA,const auto rrhs,const number accuracy)
{
  ...
  while(true)
      ...
      if constexpr (rnd)
	{
      ...
      const auto t = 0 ;
      const number rlx = 8./9. ;
      /* constexpr */ auto M = make_multigrid<std::size(rrhs),p,t,mf,gal,false>(AA);
