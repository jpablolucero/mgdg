...
constexpr auto print(const blaze::StaticMatrix<number,nc,nr> & A)
{
...
constexpr auto print(const volatile blaze::StaticVector<number,n> & v)
{
  for (const auto el : const_cast<blaze::StaticVector<number,n>& >(v))
    std::cout << el << " " ;
  std::cout << std::endl ;
}
...
template <std::size_t n,typename number=double>
constexpr blaze::StaticMatrix<number,2*n,n> prolongate_matrix()
{
  blaze::StaticMatrix<number,2*n,n> RT{};
  RT(0,0) = 1.;
  RT(1,0) = 0.5;
  RT(1,1) = 0.5;
  RT(2,0) = 0.5;
...
template <std::size_t N,typename number=double>
constexpr auto make_prolongation(blaze::StaticMatrix<number,N,N>)
{
...
public:
	  ...
	  const number rlx=1.) const
  {
	...
	x += RT*
	  vcycle0<n0,m*s,m>(std::forward<const blaze::StaticMatrix<number,nr/2,nr/2> >(R*A*RT),
	 ...
	 const number rlx=1.) const
  {
	...
	const auto & RT = std::get<logg2(N/nr)-1>(prol);
	x += RT*
	  vcycle<n0,m*s,m,prt>(std::forward<const blaze::StaticMatrix<number,nr/2,nr/2> > (R*A*RT),
			       std::forward<const blaze::StaticVector<number,nr/2> >(R * (res - A*x)),
			       rlx);
                                             // q_1 = q_0 + RT A_0^{-1} (R (g - A x_1) - A_0 R q_0)
                                             //              y_1 = x_1 + q_1
